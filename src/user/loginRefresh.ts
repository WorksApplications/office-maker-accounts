...
export async function handler( event: any ) {
  ...
  try {
    if ( 'saml' === method ) {
      return await refreshSAML(event['headers']['origin'], tenantName, event.headers['cookie'], ip, token)
    }
    if ( 'guest' === method ) {
      return await refreshGuest(event['headers']['origin'], tenantName, token)
    }
    return response(event['headers']['origin'],400, 'login method did not provided')
  } catch (e) {
    console.error(e.message || JSON.stringify(e))
    return response(event['headers']['origin'],400, e)
  }
}
...
async function refreshGuest( origin: string, tenantName: string, token: string ) {
  try {
  ...
  return response(origin,200, JSON.stringify(newToken))
}
 ...
 * @param token
 */
async function refreshSAML(origin: string, tenantName: string, cookieStr: string | undefined, ip: string, token: string ) {
  let tokenData: any
  try {
  ...
  return response(origin,200, newToken)
}
