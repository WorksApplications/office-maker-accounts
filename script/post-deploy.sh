...
cognito_user_client_callback_url="$(yq r ../config.yaml API_BASE_URL)"/saml/login/callback
...
if [[ "$cognito_admin_domain" == null ]] || [[ "$cognito_admin_domain" == "" ]];then
	...
	aws cognito-idp create-user-pool-domain --user-pool-id "$COGNITO_POOL_ADMIN" --domain \
		"$cognito_admin_domain"

	# set provider cognito
	aws cognito-idp update-user-pool-client --user-pool-id "$COGNITO_POOL_ADMIN" \
		--client-id "$COGNITO_CLIENT_ADMIN" --supported-identity-providers "COGNITO"
fi
...
if [[ "$cognito_users_domain" == null ]] || [[ "$cognito_users_domain" == "" ]];then
	...
	# add domain for client
	aws cognito-idp create-user-pool-domain --user-pool-id "$COGNITO_POOL_USER" --domain \
